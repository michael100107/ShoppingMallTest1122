package com.example.shoppingmall1122.database;
...
public class GoodsDBHelper extends SQLiteOpenHelper {
    ...
    // 往该表添加一条记录
    public long insert(PhonesInfo info) {
        ArrayList<PhonesInfo> infoArray = new ArrayList<PhonesInfo>();
        infoArray.add(info);
        return insert(infoArray);
    }

    // 往该表添加多条记录
    public long insert(ArrayList<PhonesInfo> infoArray) {
        long result = -1;
        for (PhonesInfo info : infoArray) {
    ...
    // 根据条件更新指定的表记录
    public int update(PhonesInfo info, String condition) {
        ContentValues cv = new ContentValues();
        cv.put("name", info.name);
        cv.put("desc", info.desc);
        cv.put("price", info.price);
    ...
    public int update(PhonesInfo info) {
        // 执行更新记录动作，该语句返回记录更新的数目
        return update(info, "rowid=" + info.rowid);
    }

    // 根据指定条件查询记录，并返回结果数据队列
    public ArrayList<PhonesInfo> query(String condition) {
        ...
        while (cursor.moveToNext()) {
            PhonesInfo info = new PhonesInfo();
    ...
    // 根据行号查询指定记录
    public PhonesInfo queryById(long rowid) {
        PhonesInfo info = null;
        ArrayList<PhonesInfo> infoArray = query(String.format("rowid='%d'", rowid));
        if (infoArray.size() > 0) {
